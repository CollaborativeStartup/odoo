...
# pytesseract.pytesseract.tesseract_cmd = r'C:\Program Files\Tesseract-OCR\tesseract.exe'  # Uncomment and update path if needed
...
def extract_text_from_image(image_path: str) -> str:
    """Extract text from an image using OCR with fallback to EasyOCR"""
    def use_tesseract(img_path: str) -> str:
    ...
    # Try Tesseract first
    text = use_tesseract(image_path)
    
    # If Tesseract fails or returns very little text, try EasyOCR
    if not text or len(text.strip()) < 20:  # Arbitrary threshold for "too little text"
        print("Tesseract returned little or no text, trying EasyOCR...")
        easyocr_text = use_easyocr(image_path)
        if easyocr_text:
            text = easyocr_text
    
    if not text:
        print("Warning: Both Tesseract and EasyOCR failed to extract text")
...
def upload_file():
    ...
    try:
        ...
        json_path = os.path.join(app.config['REPORTS_FOLDER'], f"{report_id}.json")
        ...
        excel_path = os.path.join(app.config['REPORTS_FOLDER'], f"{report_id}.xlsx")
        df = pd.json_normalize(report_data['expense_data']['items'])
